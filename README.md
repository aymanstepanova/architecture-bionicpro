...
- Для пользователей realm включён обязательный OTP (TOTP): при первом входе после логина/пароля пользователь обязан настроить код в Google Authenticator или FreeOTP; на следующих входах вход завершается только после ввода корректного одноразового кода.
- После `docker compose up -d` одноразовый сервис `keycloak-init` автоматически назначает `CONFIGURE_TOTP` тестовым пользователям (`user1`, `user2`, `admin1`, `prothetic1-3`), чтобы требование OTP воспроизводилось на чистом и повторном запуске стенда.
...
2. Откройте http://localhost:3000, нажмите **Login** и войдите, например как `user1` / `password123` (realm `reports-realm`).
   - Первый вход: Keycloak переведёт на шаг **Configure OTP** (сканирование QR в Google Authenticator/FreeOTP и подтверждение кода).
   - Последующие входы: после пароля обязательно появляется шаг ввода OTP; без валидного кода вход не завершается.
